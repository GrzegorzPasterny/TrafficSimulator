...
namespace TrafficSimulator.Domain.Commons
{
	...
	public static class WorldDirectionExtensions
	{
		public static WorldDirection Rotate(this WorldDirection direction, LaneType laneType)
		{
			...
			{
				LaneType.Straight => (WorldDirection)(((int)direction + 2) % 4), // Keep moving in the same direction
				LaneType.Right => (WorldDirection)(((int)direction + 3) % 4), // 90° clockwise
				LaneType.Left => (WorldDirection)(((int)direction + 1) % 4), // 90° counterclockwise
				LaneType.TurnAround => direction, // 180° turn
